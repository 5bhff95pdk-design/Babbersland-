...
| R0.4 | Gabarit de workflow **uni le 30 août** : **18 étapes** (les 15 de main — compilation, parité du portail — plus **chroniques**, **hymne** et **vignettes** rejoints depuis la PR #16), posé par `make workflows` dans `.github/workflows/continuite.yml`, scellé des archives G et H et des maîtres d'illustration, but `make workflows` | **Bloqué par un droit, non par un défaut** : le jeton d'App n'a pas `workflows`, GitHub refuse la création de `.github/workflows/*` (constat E-17, confirmé par l'API *contents* — 403). Activation = 1 commit depuis un compte habilité, ou droit *Workflows : Read and write* accordé à l'App (deux voies documentées au README). **Amorce anticipée de R1.1** (sans elle, la CI régénérait le PDF et le jetait : un volume publié périmé passait) |
