...
| **R1.4.a** | **Empreinte sémantique de l'Atlas** | Retirer `continue-on-error: true` de l'étape Atlas. Implémenter `sources/empreinte_atlas.py` qui calcule un hash sémantique du SVG/PNG/HTML (ex. : trie les chemins SVG, ignore les métadonnées Pillow, normalise le HTML). Stocker l'empreinte dans `gouvernance/ARTIFACT_SIGNATURES.sha256`. | 1 h | R1.1 |
