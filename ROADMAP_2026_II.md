...
| **R1.4.b** ✅ | **Empreinte sémantique de l'Arbre** | **Livré le 1ᵉʳ septembre 2026** : `sources/empreinte_arbre.py` (~85 lignes), empreinte SHA-256 sémantique du PNG conçue **plus tolérante** que celle de l'Atlas (leçon de R1.4.a-v2) : moyennage **BOX** 16×16 quantifié en 16 niveaux (au lieu de NEAREST) + proportion d'encre au millième + géométrie. Section dédiée dans `gouvernance/ARTIFACT_SIGNATURES.sha256`, but `make empreinte-arbre`, `--check` dans `make controle`. Étape CI **bloquante** : `continue-on-error: true` et `git diff --exit-code` retirés ; en cas de divergence, le script imprime charge, sha256 du fichier et version Pillow (diagnostic lisible dans le log, la douleur de v2). Tests rejoués : regen conforme ; nœud ajouté et titre gommé détectés (code 1) ; 1 pixel et bruit ±2 tolérés. | 30 min | R1.4.a |
...
**Reste ouvert** : R1.2 (matrice macOS/Windows), R1.3 (manifeste complet), R1.4 (CI de bout en bout — l'ossature est là ; **R1.4.a et R1.4.b durcis le 1ᵉʳ septembre**, restent R1.4.c–h : Hymne, Vignettes, PDF, gel des archives), R1.5 (**re-poser `v2026-I` sur `main` + Release**, à faire avec un accès aux étiquettes), R1.6 (maîtres PNG hors dépôt), R1.7 (source unique de l'arbre), l'horizon 2 sauf **R2.6 et R2.8 livrés en proposition** (atlas temporel, Livre II fusionné et Chroniques Livres III–IV, 2026-I intact, Avis n° 7 restant — voir `geographie/ROADMAP.md`). Suite à prévoir : **Livre VII des Chroniques** (le Livre VI, livré en proposition, s'arrête en 2026).
