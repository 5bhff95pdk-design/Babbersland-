...
### R0.4 · Activer réellement la CI `P1` ⏳ prêt, bloqué par un droit de jeton (E-17)
- **Constat** : E-14.1 — `sources/github_actions_continuite.yml` dort en « proposition », `.github/workflows/` n'existe pas, alors que c'est le seul garde-fou automatique contre E-07.
- **Action** : `make workflows` (copie du modèle vers `.github/workflows/continuite.yml`), puis l'étendre au fil des horizons ; conserver le fichier sous `sources/` comme modèle documenté et unique source de vérité.
- **Effort** : 1 commit · 5 min · **Fait le jour où** : une exécution du workflow apparaît dans l'onglet Actions et que la pull request qui l'ajoute est verte.
- **Obstacle mesuré (E-17)** : GitHub refuse qu'un jeton d'application *sans* la permission `workflows` crée un fichier dans `.github/workflows/`. Le `git push` est donc rejeté, ici comme lors des sessions précédentes. Deux issues : accorder le droit à l'App, ou committer ce fichier à la main. En attendant, `make controle` exécute localement les mêmes étapes.
- **Re-mesuré le 29 août 2026** : le refus est double — `git push` (« refusing to allow a GitHub App to create or update workflow … without `workflows` permission ») **et** API *contents* en 403 « Resource not accessible by integration ». Le poussage des autres fichiers de la branche réussit : le blocage est strictement limité au droit `workflows`. **Fait le jour où** : une exécution verte apparaît dans l'onglet Actions.
...
| **R1.6** | **Iconographie : maîtres hors dépôt** | Mesures : 37,2 Mio de PNG → 9,4 Mio quantifiés (−75 %, dégradation) → 4,7 Mio en JPEG q90 (−87 %, dégradation) → **0 %** en gardant les maîtres hors dépôt et en laissant `prepared_image()` produire les dérivés. Décision recommandée : transférer les maîtres dans les Releases (ou LFS), commit de versions compressées dédiées à l'écran ; le SVG source n'est pas affecté. | 3 h | R1.3 |
...
| **R2.8** ✅ | **Chroniques, Livre II — *Les Bâtisseurs* (1889–1914)** | Livre I couvre 1798–1889 en 7 tranches et laisse volontairement l'Article 4 au Dormeur. Le Livre II naturel : régence posthume de la mémoire de Babette, plans de l'aqueduc, mort de François-Babber, avènement du Dormeur. Garder le bandeau « proposé, non décrété » + le même audit d'arithmétique interne (12 calculs, 0 erreur sur le Livre I : c'est le standard à reproduire). **Une seconde rédaction du Livre II (*Le Silence et l'Aqueduc*, 28 août 2026) est versée le lendemain : voir le suivi d'exécution — l'Avis tranchera.** | 4-5 j | — |
...
| R0.4 | Gabarit de workflow **remis à jour le 29 août** : **14 étapes** (l'hymne, décrété par l'Avis n° 8 et entré dans `make tout`, manquait à la CI), posé par `make workflows` dans `.github/workflows/continuite.yml` (parité des données et artéfact apparié depuis RC-2026-III-01), scellé des archives G et H et des maîtres d'illustration | **Bloqué par un droit, non par un défaut** : le jeton d'App n'a pas `workflows`, GitHub refuse la création de `.github/workflows/*` (constat E-17, confirmé aussi via l'API le 29 août). Activation = 1 commit depuis un compte habilité, ou droit *Workflows : Read and write* accordé à l'App. **Amorce anticipée de R1.1** (sans elle, la CI régénérait le PDF et le jetait : un volume publié périmé passait) |
...
| R2.8 | Livré en proposition le 27 août 2026 : `chroniques/LIVRE_II_LES_BATISSEURS.md`, 7 tranches, 1889–1914, bandeau « proposé, non décrété » conforme, **2026-I intact** | Le titre retenu est celui que la fin du Livre I annonçait déjà — ***Les Bâtisseurs*** — et non le titre de travail de la roadmap (*Le Silence et l'Aqueduc*) ; la chronique a priorité sur la fiche. Aucune lacune volontaire comblée : filiation de Babette-Marine, naissance de Roger Bontemps et Parité Poutine chiffrée restent ouvertes et sont nommément déclarées telles dans le texte |
| R2.8 (suite) | **Seconde rédaction livrée en proposition le 28 août 2026** : `chroniques/LIVRE_II_LE_SILENCE_ET_L_AQUEDUC.md`, 7 tranches, 1889–1914, bandeau conforme, **2026-I intact**, + **4 planches de chantier** (`images/aqueduc_*.png`, Annexe C du livre, hors volume comme lui) | L'archive détient désormais **deux rédactions rivales du Livre II** : *Les Bâtisseurs* (titre du colophon du Livre I) et *Le Silence et l'Aqueduc* (titre de travail de la roadmap). Aucune contradiction canonique — les deux respectent les mêmes points de continuité — mais des divergences de proposition : nombre de bancs (**40 contre 42**), cotes d'archives, palette d'événements et jour de la mise en eau. **L'Avis qui ratifiera le Livre II choisira ou mariera les deux rédactions** ; d'ici là, aucune n'entre au canon |
...
**Reste ouvert** : R1.2 (matrice macOS/Windows), R1.3 (manifeste complet), R1.4 (CI de bout en bout — l'ossature est là), R1.5 (**re-poser `v2026-I` sur `main` + Release**, à faire avec un accès aux étiquettes), R1.6 (maîtres PNG hors dépôt), R1.7 (source unique de l'arbre), l'horizon 2 sauf **R2.6 et R2.8 livrés en proposition** (atlas temporel, Chroniques Livres II à IV, 2026-I intact, Avis n° 7 restant — voir `geographie/ROADMAP.md`). Suite à prévoir : **Livre V — L'Union des Règnes (1998–2010)**, annoncé en fin de Livre IV.
