...
| R0.4 | Gabarit de workflow **remis à jour le 29 août** : 13 étapes, posé par `make workflows` dans `.github/workflows/continuite.yml` (parité des données et artéfact apparié depuis RC-2026-III-01), scellé des archives G et H et des maîtres d'illustration, but `make workflows` | **Bloqué par un droit, non par un défaut** : le jeton d'App n'a pas `workflows`, GitHub refuse la création de `.github/workflows/*` (nouveau constat E-17). Activation = 1 commit depuis un compte habilité. **Amorce anticipée de R1.1** (sans elle, la CI régénérait le PDF et le jetait : un volume publié périmé passait) |
