...
| **R1.4.a** | **Empreinte sémantique de l'Atlas** | ✅ **Livré le 1ᵉʳ septembre 2026** (PR #24) : diagnostic refait sur un clone frais avec `git hash-object`. L'Atlas (SVG, PNG, HTML) est en fait **bit-à-bit reproductible** — la mesure initiale (confusion `sha256sum` filesystem vs `git hash-object`) était fausse. `continue-on-error: true` retiré, étape désormais bloquante. Voir `gouvernance/CI_LIMITES.md` § « Statut R1.4.a ». | — | — |
