...
**Reste ouvert** : R1.2 (matrice macOS/Windows — c'est elle qui rendrait caduques les cérémonies d'acceptation de variantes, en les prévenant en amont), R1.3 (manifeste complet), **R1.4 clos le 1ᵉʳ septembre 2026** (R1.4.a–h livrés ; 18 étapes, 0 tolérante), **R1.8 et R1.9 clos le 1ᵉʳ septembre 2026** (nés de la vague R1.4 et fermés dans la foulée : parité modèle↔workflow en 19ᵉ étape bloquante, galerie scellée par `GALERIE.sha256` ; batterie à 27 scénarios), R1.5 (**re-poser `v2026-I` sur `main` + Release**, à faire avec un accès aux étiquettes), R1.6 (maîtres PNG hors dépôt), R1.7 (source unique de l'arbre), l'horizon 2 sauf **R2.6 et R2.8 livrés en proposition** (atlas temporel, Livre II fusionné et Chroniques Livres III–IV, 2026-I intact, Avis n° 7 restant — voir `geographie/ROADMAP.md`). Suite à prévoir : **Livre VII des Chroniques** (le Livre VI, livré en proposition, s'arrête en 2026).
