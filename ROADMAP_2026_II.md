...
| **R1.4.a** | **Empreinte sémantique de l'Atlas** | ✅ **Livré R1.4.a-v2 le 1ᵉʳ septembre 2026** (PR #25) : `sources/empreinte_atlas.py` (60 lignes) calcule trois empreintes SHA-256 sémantiques (SVG : viewBox/ids/data-since/classes/toponymes ; PNG : dim/mode/MD5 pixels ; HTML : ids/classes/h1/h2/dates). Stockage dans `gouvernance/ARTIFACT_SIGNATURES.sha256`. Workflow : `python sources/empreinte_atlas.py --check` au lieu de `git diff --exit-code`. Étape désormais bloquante. Voir `gouvernance/CI_LIMITES.md` § « Statut R1.4.a-v2 ». | — | R1.1 |
