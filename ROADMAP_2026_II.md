...
| **R2.6** | **Démographie et géographie chiffrées** | E-16.c : totaliser la population, chiffrer les Monts Froissés et la Forêt, assumer l'écart des 7 000 mini-McLouches (recommandation : 7 000 = « un par âme connue dans la capitale et ses deux villes, plus les sujets de la forêt » → en faire une **blague de registre** consignée, pas une incohérence). Ajouter une **carte du Royaume** (SVG vectoriel, même pipeline que l'arbre : R1.7 sert ici). | 1 j | tonalité |
...
**Reste ouvert** : R1.2 (matrice macOS/Windows), R1.3 (manifeste complet), R1.4 (CI de bout en bout — l'ossature est là), R1.5 (**re-poser `v2026-I` sur `main` + Release**, à faire avec un accès aux étiquettes), R1.6 (maîtres PNG hors dépôt), R1.7 (source unique de l'arbre), tout l'horizon 2.
